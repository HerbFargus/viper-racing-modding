...
THE ORDER, AND WHY IT IS THIS ORDER:
    ...
    5c. fixes     Engine bug fixes, always on (enginefix): obstacles wake on a restart,
                  and the AI survives losing its racing line. On race.bin the AI guard
                  puts a stub in the .text slack, found the same way needle's is.
...
def status(data_dir: str | Path) -> dict[str, str]:
    ...
    return {
        ...
        "fixes": ", ".join(f"{k} {v}" for k, v in enginefix.status(d).items()),
        "resolution": modes,
...
def apply(data_dir: str | Path,
    ...
    # 5c -- engine bug fixes, on every install: obstacles wake on a restart, and the AI
    #       survives losing its racing line. Neither changes anything when things go
    #       right, so there is no switch. Before the map, which has to see final code.
    blob = bytearray(f.read_bytes())
    try:
        for k, v in enginefix.apply_bytes(blob).items():
            rep.add(f"fixes/{k}", v)
        safewrite.write_atomic(f, bytes(blob))
    except enginefix.EngineFixError as ex:
        rep.notes.append(f"Engine bug fixes skipped: {ex}")
