...
Produces a **single file**, `dist\ViperModManager.exe` (~17MB) — that one file
*is* the whole app; hand it to anyone. The spec bundles `vrmod/assets/`
(three.js + slot icons), capstone's native lib, pywebview's backend, and the app
icon (`icon.ico`, generated by `make_icon.py`). One-file unpacks to a temp dir on
launch, so first start is a beat slower than a one-folder build. To see a startup
traceback while debugging, flip `console=False` to `True` in the spec.
...
- Phase 3 (packaging) — done: `viper-mod-manager.spec` builds a single-file
  `dist\ViperModManager.exe` (~17MB) with the app icon; three.js, slot icons,
  capstone.dll, and pywebview's backend all bundled. See "Building the standalone .exe".
