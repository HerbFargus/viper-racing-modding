...
- **Obstacles in the race path can crash the game when AI cars race.** Three generated tracks have
  ...
  **The fix:** have `advance_bead` put a NULL bead back at the head of the line (`+0x2c`, t = 0), and the
  ...
  32-byte profiler preamble. The `race.bin` builds (1.0 disc, 1.1, 1.2.4 beta, 1.2.5, 1.2.6) were compiled
  without the profiler, so there its first two loads become a jump to a 26-byte stub in the `.text` slack.
  Both forms, and the restart fix above, ship in the patch set (`vrmod/enginefix.py`).
...
```
vrmod patch <Data> --mode 1920x1080     # startup, rasteriser, FOV, resolution, engine fixes, symbols
...
rather than a change to any game file. The **engine fixes** (`vrmod/enginefix.py`) are always on: obstacles
wake on a race restart, and the AI survives losing its racing line (both under "How an obstacle lives").
Each is found by signature in every known build: v1.0 `race.exe`, the v1.0-disc and v1.1 `race.bin`, and
1.2.4 beta, 1.2.5 and 1.2.6. `scripts/check_enginefix.py` checks them against each.
